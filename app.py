...
def build_kb():
    ...
    result = build_knowledge_base(course_id, materials_dir)
...
def setup_assignment():
    ...
    if rubric_file:
        rubric_path = os.path.join(assignment_dir, "rubric.txt")
        rubric_file.save(rubric_path)
        saved.append("rubric.txt")

    solution_file = request.files.get("solution")
    if solution_file:
        solution_path = os.path.join(assignment_dir, "solution.pdf")
